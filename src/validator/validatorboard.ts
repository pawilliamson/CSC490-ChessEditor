...
export class ValidatorBoard extends Board {
  ...
  goodDiagonalPathChecker(x1: number, y1: number, x2: number, y2: number) {
    ...
    if (xChange === -yChange || -xChange === yChange || xChange === yChange) {
      ...
      let tempy: number = y1;
      for (
        tempx = tempx + (xChangePerSpace > 0 ? 1 : -1);
        tempx !== x2;
        tempx += xChangePerSpace > 0 ? 1 : -1
      ) {
   ...
   *
   */
  goodHorizontalVerticalPathChecker(
    x1: number,
    y1: number,
    x2: number,
    y2: number
    ...
    if (xChange === 0 || yChange === 0) {
      ...
      if (yChange === 0) {
        for (
          let tem = tempx + (xChangePerSpace > 0 ? 1 : -1);
          tem !== x2;
      ...
      if (xChange === 0) {
        for (
          let temy = tempy + (yChangePerSpace > 0 ? 1 : -1);
          temy !== y2;
  ...
  checkQueenMovement(x1: number, y1: number, x2: number, y2: number) {
    ...
    if (
      this.goodDiagonalPathChecker(x1, y1, x2, y2) ||
