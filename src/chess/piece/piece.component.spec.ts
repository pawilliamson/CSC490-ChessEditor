...
import { Piece, Pawn, Rook, Bishop, ValidatorBoard, Knight, Queen, King, Board
}from '../../validator/validator.module';
describe('PieceComponent', () => {
	let component: PieceComponent;
	let pieceComponentMock: PieceComponent;
	let piece: Piece;
	const xStartPosition = 3; const yStartPosition = 3;
	const boardStart = 0; const boardEnd = 7;
	...
	describe('Pawns', () => {
		...
		it('should move correctly when white', () => {;
			...
			expect(board.validateMovement(xStartPosition,
				yStartPosition, xStartPosition, yStartPosition
				+ 1)).toBeTrue();
		});
		...
		it('should be able to capture an opposing piece', () => {
			...
			expect(board.validateMovement(xStartPosition,
				yStartPosition, xStartPosition + 1,
				yStartPosition + 1)).toBeTrue();
		});
	...
	describe('Rooks', () => {
		...
		it('should be able to move horizontally', () => {
			...
			expect(board.validateMovement(xStartPosition,
				yStartPosition, boardEnd,
				yStartPosition)).toBeTrue();
		});
		...
		it('should not be able to move diagonally', () => {
			const diag = xStartPosition > yStartPosition ? xStartPosition - 1 : yStartPosition - 1;
			board.chessBoard[xStartPosition][yStartPosition] = rook;
			expect(board.validateMovement(xStartPosition,
				xStartPosition, xStartPosition - diag,
				yStartPosition - diag)).toBeFalse();
		});
		...
		it('should not be able to pass through another piece to get to a cell', () => {
			...
			expect(board.validateMovement(xStartPosition,
				yStartPosition, xStartPosition+2,
				yStartPosition)).toBeFalse();
		});
	});
	...
	describe('Bishops',()=> {
		...
		it('should be able to move diagonally', () => {
			...
			board.chessBoard[xStartPosition][yStartPosition] = bishop;
			expect(board.validateMovement(xStartPosition,
				xStartPosition, xStartPosition + 1,
				yStartPosition - 1)).toBeTrue();
		});
